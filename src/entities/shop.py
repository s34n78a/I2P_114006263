...
@dataclass
class IdleMovement:
    def update(self, shop_keeper: "ShopKeeper", dt: float) -> None:
        return

class ShopKeeper(Entity):
    classification: ShopKeeperClassification
    max_tiles: int
    ...
    def __init__(
        ...
        classification: ShopKeeperClassification = ShopKeeperClassification.STATIONARY,
        max_tiles: int = 2,
        ...
        if classification == ShopKeeperClassification.STATIONARY:
            self._movement = IdleMovement()
            if facing is None:
                raise ValueError("Idle ShopKeeper requires a 'facing' Direction at instantiation")
    ...
    def _get_los_rect(self) -> pygame.Rect | None:
        ...
        # checkpoint 2
        # LOS panjangnya max_tiles & lebar 1 tile dari posisi shop keeper ke atas kiri kanan bawah
    ...
    def from_dict(cls, data: dict, game_manager: GameManager) -> "ShopKeeper":
        classification = ShopKeeperClassification(data.get("classification", "stationary"))
        ...
        if facing_val is not None:
            if isinstance(facing_val, str):
                ...
                facing = facing_val
        if facing is None and classification == ShopKeeperClassification.STATIONARY:
